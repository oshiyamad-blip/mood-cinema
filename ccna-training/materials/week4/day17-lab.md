...
使用機器: Router ISR 4331（または 2911）× 1 = R1、L2 スイッチ 2960 × 1 = SW1、
...
3. VTY 制限にインターフェースの `ip access-group` ではなく line vty の
   ...
   PC-Adm と PC-Kei の SSH の結果を根拠に説明してください
...
| PC-Kei から SSH が成功してしまう | `access-class MGMT-VTY in` が正しい line vty に適用されているか、`show ip interface` ではなく `show line` 系で確認が必要な点に注意 |
