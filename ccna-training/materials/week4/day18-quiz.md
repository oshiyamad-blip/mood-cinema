...
> ルール: 12 問 / 35 分 / 教材参照なし。解答はコメントに「Q1: A」形式で提出。
...
**Q11.** ESP・AH のプロトコル番号、および IKE・NAT-T が使用するポート番号の
組み合わせとして正しいものはどれか。

- A. ESP=50 ／ AH=51 ／ IKE=UDP 500 ／ NAT-T=UDP 4500
- B. ESP=51 ／ AH=50 ／ IKE=UDP 500 ／ NAT-T=UDP 4500
- C. ESP=50 ／ AH=51 ／ IKE=TCP 500 ／ NAT-T=TCP 4500
- D. ESP=50 ／ AH=51 ／ IKE=UDP 4500 ／ NAT-T=UDP 500

**Q12.**（シナリオ）SW1 で DHCP スヌーピングを有効化した直後から、フロア
全体のクライアントが IP アドレスを取得できなくなった。最も可能性が高い
原因はどれか。

- A. `ip dhcp snooping limit rate` の値を大きくしすぎている
- B. DHCP サーバ／リレー方向へのアップリンクポートを `trust` に設定して
  いない
- C. `ip arp inspection vlan` を設定し忘れている
- D. sticky MAC アドレスを保存していない
...
| Q11 | A | ESP=50、AH=51（ESP の次の番号）。IKE は UDP 500、NAT 越え時の NAT-T は UDP 4500 を使用する（TCP ではない） |
| Q12 | B | DHCP スヌーピング有効化後は untrusted ポートでサーバ発メッセージが破棄されるため、DHCP サーバ／リレー方向のアップリンクを `trust` にし忘れると全クライアントが IP を取得できなくなる |

**採点**: 1 問 10 点（120 点満点）、84 点未満（70% 未満）は翌朝再テスト。
Q10 は趣旨が合っていれば 10 点。
