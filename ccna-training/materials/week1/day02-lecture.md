...
- 特権 EXEC から `disable` を実行するとユーザ EXEC に戻ります（`exit` でも同様に
  ユーザ EXEC に戻ります）。
...
インタフェースは工場出荷時・初期化直後は **shutdown（無効化）状態**であることが
一般的で、特にルータの物理インタフェースは既定で **administratively down（管理者に
よって無効化された状態）**になっています。`no shutdown` を忘れると、IP アドレスを
正しく設定していても通信できません。
