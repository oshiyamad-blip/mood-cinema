...
1. PC1 から PC3 へ `ping 192.168.10.13` を複数回（`ping 192.168.10.13 -n 10` など）
   実行し、応答時間の増加やパケットロス（Request timed out）が発生していないかを
   観察して記録する
2. 手順 7 で確認したエラーカウンタが ping 実行後にさらに増加していないかを
   再度 `show interfaces gigabitEthernet0/1` で確認する

## 手順 9: 復旧（10 分）

1. SW1 の `GigabitEthernet0/1` を元のオートネゴシエーションに戻す
   ...
   （または、両端を同一の固定値 `speed 100` / `duplex full` に揃える方法でも
   不一致は解消します。今回は両端を auto に統一する方法で復旧します）
...
2. デュプレックス / 速度不一致を発生させたとき、`show interfaces status` の表示と
   増加したエラーカウンタ（種類）は何だったか。ping の結果にどのような影響が
   出たかを記述せよ。
...
| 障害演習後に ping が完全に通らなくなった | 片方だけ固定・片方 auto という不一致の組み合わせになっているか、意図した設定通りか再確認する |
