...
**Q4.** IPv4 のデフォルトルートを表す宛先ネットワークとサブネットマスクの
組み合わせとして正しいものはどれか。
...
**Q19.** DHCP プールで、配布対象のネットワーク範囲を指定するコマンドはどれか。
...
**Q20.** NTP が使用するポート番号と、ルータを基準時刻源として動作させる
コマンドの組み合わせとして正しいものはどれか。

- A. UDP 161、ntp master
- B. UDP 123、ntp server
- C. TCP 123、ntp peer
- D. UDP 123、ntp master

**Q21.** Syslog の severity レベルのうち、数値 4 に対応する名称はどれか。
...
**Q22.** SNMP に関する記述として正しいものはどれか。

- A. SNMPv2c はコミュニティストリングを暗号化して送信する
- B. SNMPv3 は認証と暗号化に対応し、v1/v2c より安全性が高い
- C. マネージャからのポーリングは UDP 162、エージェントからの Trap は UDP 161 を使用する
- D. Trap はマネージャからの確認応答を必ず受け取る
...
- C. crypto key generate rsa のみで自動的に Telnet は無効化される
- D. ip domain-name の設定 → crypto key generate rsa → line vty での transport input ssh
...
| Q4 | D | デフォルトルートは宛先 `0.0.0.0`、マスク `0.0.0.0`（プレフィックス長 0）で表す |
...
| Q19 | C | `network` コマンドで DHCP プールが配布対象とするネットワーク範囲を指定する |
| Q20 | D | NTP は UDP 123 を使用する。自身を基準時刻源にするコマンドは `ntp master <stratum>` |
| Q21 | A | severity 4 は Warning（警告）。0 Emergency から 7 Debugging まで数値が小さいほど重大 |
| Q22 | B | SNMPv3 は認証・暗号化に対応し v1/v2c より安全。v1/v2c のコミュニティストリングは平文、ポーリングは UDP 161、Trap は UDP 162 が正しい対応 |
| Q23 | C | シェーピングは超過分をバッファに貯めて遅延させ送出レートを平滑化する。ポリシングは即時に破棄・再マークしバッファしない |
| Q24 | D | SSH 有効化には ip domain-name の設定、crypto key generate rsa による鍵生成、line vty での transport input ssh（および login local）の設定が必要 |
