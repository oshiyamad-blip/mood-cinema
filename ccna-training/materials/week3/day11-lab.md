...
```
                 主経路（10.0.12.0/30・2001:db8:12::/64）
PC1 ──(Gi0/0)── R1 ──(Gi0/1)────────(Gi0/0)── R2 ──(Gi0/1)────────(Gi0/0)── R3 ──(Gi0/1)── PC3
LAN1                │                                                       │        LAN3
192.168.1.0/24      │                                                       │        192.168.3.0/24
2001:db8:1::/64     │        バックアップ経路（10.0.13.0/30・2001:db8:13::/64）│        2001:db8:3::/64
                     └──────────────────(Gi0/2)────(Gi0/2)───────────────────┘
```
...
| R1 | Gi0/1 | 10.0.12.1/30 | 2001:db8:12::1/64 | 主経路（R2 方向） |
| R1 | Gi0/2 | 10.0.13.1/30 | 2001:db8:13::1/64 | バックアップ経路（R3 方向） |
| R2 | Gi0/0 | 10.0.12.2/30 | 2001:db8:12::2/64 | 主経路（R1 方向） |
| R2 | Gi0/1 | 10.0.23.1/30 | 2001:db8:23::1/64 | 主経路（R3 方向） |
| R3 | Gi0/0 | 10.0.23.2/30 | 2001:db8:23::2/64 | 主経路（R2 方向） |
| R3 | Gi0/1 | 192.168.3.1/24 | 2001:db8:3::1/64 | LAN3 側 |
| R3 | Gi0/2 | 10.0.13.2/30 | 2001:db8:13::2/64 | バックアップ経路（R1 方向） |
...
4. ストレートケーブルで次のように接続する
   ...
   - R2 Gi0/1 ⇔ R3 Gi0/0
   - R3 Gi0/1 ⇔ PC3
   - R1 Gi0/2 ⇔ R3 Gi0/2（バックアップリンク。この時点では接続のみ行い、
     設定は手順 8 まで保留してもよい）
...
1. R1 に、遠方の LAN3（`192.168.3.0/24`）向けの静的ルートを設定する

   ```
   R1(config)# ip route 192.168.3.0 255.255.255.0 10.0.12.2
   ```

2. R3 に、復路となる LAN1（`192.168.1.0/24`）向けの静的ルートを設定する

   ```
   R3(config)# ip route 192.168.1.0 255.255.255.0 10.0.23.1
   ```

3. R2 に、両端の LAN 向けの静的ルートを設定する
...
1. R1 に IPv6 の静的ルートを設定する

   ```
   R1(config)# ipv6 route 2001:db8:3::/64 2001:db8:12::2
   ```

2. R3 に復路の IPv6 静的ルートを設定する
...
3. **確認**: `tracert` の結果が `R1 → R2 → R3` の順にホップしていることを記録する
...
## 手順 8: バックアップリンクの有効化（15 分）

1. R1 Gi0/2 と R3 Gi0/2 に IP アドレス表の値でアドレスを設定し、`no shutdown` する
   ...
   R3 側も同様に `10.0.13.2` / `2001:db8:13::2/64` を設定する

2. 両ルータで `show ip interface brief` を実行し、Gi0/2 が `up`/`up` に
   なっていることを確認する

## 手順 9: フローティングスタティックの設定（15 分）

1. R1 に、LAN3 向けのフローティングスタティック（AD=5）を設定する

   ```
   R1(config)# ip route 192.168.3.0 255.255.255.0 10.0.13.2 5
   R1(config)# ipv6 route 2001:db8:3::/64 2001:db8:13::2 5
   ```

2. R3 に、LAN1 向けのフローティングスタティック（AD=5）を設定する

   ```
   R3(config)# ip route 192.168.1.0 255.255.255.0 10.0.13.1 5
   R3(config)# ipv6 route 2001:db8:1::/64 2001:db8:13::1 5
   ```
...
1. R1 で `show ip route` を実行し、`192.168.3.0/24` 宛の経路が
   **主経路（via 10.0.12.2）のみ**表示され、フローティング側（via 10.0.13.2）は
   表示されていないことを確認する
...
2. `show ip route` を再実行し、フローティングスタティック（via 10.0.13.2）が
   テーブルに昇格したことを確認する
...
2. `show ip route` を再実行し、主経路（via 10.0.12.2）がテーブルに復帰し、
...
2. `show running-config | include ip route` で、静的ルート・デフォルトルート・
...
2. R1 の Gi0/1 を `shutdown` した前後で `show ip route` の該当行がどう変化したか
