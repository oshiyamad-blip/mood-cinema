...
マルチアクセス環境（イーサネットなど）で静的ルートを設定する場合は、この
**完全指定**が推奨されます。
...
5. マルチアクセス環境（イーサネット）で静的ルートを設定する際に推奨される指定方法は何か。またその理由を答えよ。
...
5. 完全指定ルート（出力インターフェース＋ネクストホップ IP の両方を指定する方法）が推奨される。出力インターフェースのみの指定では、ネクストホップが特定できず ARP に関する問題が発生しうるため。
