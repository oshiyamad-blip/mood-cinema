...
> **事前準備メモ**: 既定の Cisco 2911 は搭載済みの Gigabit ポートが `Gi0/0`・`Gi0/1`
> の 2 つのみです。R1 と R2 は 3 本目のリンク（`Gi0/2`）が必要なので、電源を切った
> 状態で拡張モジュール（Gigabit Ethernet 系の NM モジュール）を空きスロットに
> ドラッグ＆ドロップで追加し、電源を入れ直してから作業を始めてください。
...
1. `show ip ospf neighbor` を R1・R2 双方で実行し、DR が R1 に変わったことを確認する
