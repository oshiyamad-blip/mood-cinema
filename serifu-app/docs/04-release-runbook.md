...
本アプリは **MVP 完全オンデバイス**（台本・音声を端末外へ送信しない）。この事実を強調する。

- [ ] **App Store Connect → App Privacy**：
  - データ収集：**「データを収集していません（Data Not Collected）」**を選択（解析 SDK・広告 SDK を入れていない前提）。
  - マイク/音声認識を使うが**端末内処理で外部送信しない**ため、「収集」には該当しない（収集＝サーバ送信を伴うもの）。
- [ ] **Google Play → アプリのコンテンツ → データセーフティ**：
  - データ収集・共有：**なし**（オンデバイス処理のみ）。
  - データの暗号化（転送中）：該当する送信がなければ「なし」だが、フォーム上は適切に申告。
  - 「データはオンデバイスで処理される」旨を記載。

> 将来クラウド TTS/LLM を**オプトイン**で追加した場合は、両フォームを更新し、送信データ種別・第三者・学習非利用を明記すること（仕様書 §3-8）。
